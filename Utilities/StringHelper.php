<?php
	...
	class StringJoinOptions {
		...
		public string $glue;
		/**
		 * Whether to guard against glue duplicates when joining.
		 *
		 * @var bool
		 */
		public bool $guardGlue = false;
		 ...
		 * @param bool $guardGlue Whether to discourage duplication of glue string.
		 */
	...
	/**
	 * Class to provide some basic helper methods for working with strings.
	 *
	 * @package Stoic\IO
	 * @version 1.1.0
	 */
	class StringHelper {
		...
		/**
		 * Instantiates a new StringHelper object with an optional source string.
		 *
		...
		/**
		 * Appends a string onto the internal data store or initializes the store if empty.
		 *
		 * @param string|StringHelper $string String to append to (or initialize as) internal data store.
		 * @return void
		 */
		public function append(string|StringHelper $string) : void {
		 ...
		 *
		 * @param int $position Value of position to retrieve character within string.
		...
		/**
		 * Binary safe comparison of two strings, with optional specifier for comparing the first $length characters.
		 *
		 * @param string|StringHelper $string Another string (or string object) to compare with.
		 * @param null|int $length Optional position of first character in internal data store to use for comparison.
		 * @param bool $caseInsensitive Optional toggle for case sensitivity in comparison, defaults to sensitive.
		 * @return bool|int
		 */
		public function compare(string|StringHelper $string, ?int $length = null, bool $caseInsensitive = false) : bool|int {
			if ($this->_data === null) {
				return false;
			}
		 ...
		 * @param bool $caseInsensitive Optional toggle for case sensitivity of comparison.
		 * @return bool
		 */
		...
		/**
		 * Attempts to find a string within the internal data store, with optional position.
		 *
		 * @param string|StringHelper $string Another string (or string object) to find within store.
		 * @param null|int $position Optional position of first character in internal data store to use for searching.
		 * @param bool $caseInsensitive Optional toggle for case sensitivity in searching, defaults to sensitive.
		 * @return bool|int
		 */
		public function find(string|StringHelper $string, ?int $position = null, bool $caseInsensitive = false) : bool|int {
			if ($this->_data === null) {
				return false;
			}
		...
		/**
		 * Retrieves the first character in the internal data store.
		 *
		 * @return null|string
		 */
		...
		/**
		 * Returns true if the internal data store is null or empty.
		 *
		 * @return bool
		 */
		...
		/**
		 * Returns true if the internal data store is null, empty, or comprised of only whitespace.
		 *
		 * @return bool
		 */
		...
		/**
		 * Retrieves the last character in the internal data store.
		 *
		 * @return null|string
		 */
		 ...
		 * @return void
		 */
		public function replace(mixed $search, mixed $replace, mixed &$count = null) : void {
			if ($count !== null) {
		...
		/**
		 * Finds and replaces text contained within the $start and $end tags with the $replace sequence.  Use of the text
		 * between $start and $end is signified in the $replace string using the string '%TEXT%'.
		 *
		 ...
		 * @param bool $caseInsensitive Optional toggle of case sensitivity for searching.
		 * @return bool
		 */
		public function replaceContained(string|StringHelper $start, string|StringHelper $end, string|StringHelper $replace, bool $caseInsensitive = false) : bool {
			$offset = 0;
			$endLen = strlen($end);
		...
		/**
		 * Tries to find and replace the search string inside the internal data store only once.
		 *
		 ...
		 * @return bool
		 */
		public function replaceOnce(string|StringHelper $search, string|StringHelper $replace, mixed $position = 0, bool $caseInsensitive = false) : bool {
		 ...
		 * @return bool
		 */
		public function startsWith(string|StringHelper $string, bool $caseInsensitive = false) : bool {
			$findFunction = ($caseInsensitive === false) ? self::CMP_STRPOS : self::CMP_STRIPOS;
		 ...
		 *
		 * @param int $start Integer value to start substring selection based on position.
		 * @param null|int $length Optional integer value to limit returned substring length.
		 * @return string
		 */
		...
		/**
		 * Overrides default toString() behavior to instead output the internal data store.
		 *
		 * @return string
		 */
		...
		/**
		 * Joins strings together using the first argument as the glue string.  First argument can also optionally be a
		 * StringJoinConfig object.
		 *
